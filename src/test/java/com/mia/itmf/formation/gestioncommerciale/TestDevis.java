...
public class TestDevis {
	...
	public static void init() throws ExceptionClient, ExceptionProduit, ExceptionDocument{
		...
		devis = gestionDevis.ajouterDevis(new Devis(gestionClient.retrouverClient(2)));
		devis.ajouterLigneDetail(new DetailDocument(1, 12, gestionProduit.retrouverProduit(1)));
		devis.ajouterLigneDetail(new DetailDocument(2, 15, gestionProduit.retrouverProduit(2)));
		devis.ajouterLigneDetail(new DetailDocument(3, 20, gestionProduit.retrouverProduit(3)));
		
	}
	
	//@Disabled
	@Test @Order(1)
	public void test1_afficherDevis(){
		assertEquals(devis.toString(), "Devis-1, Nom du client: THOMAS Paul Montant: 3150.0");
		assertEquals(devis.getMontant(),3150.0);
		assertEquals(devis.getListeDetail().size(),3);
	}
	
	//@Disabled
	@Test @Order(2)
	public void test2_ajouterLigneDevis1() {
		try {
			devis.ajouterLigneDetail(new DetailDocument(4, 50, gestionProduit.retrouverProduit(4)));
		} catch (ExceptionDocument e) {
			e.printStackTrace();
		}
		assertEquals(devis.toString(),"Devis-1, Nom du client: THOMAS Paul Montant: 5150.0");
		assertEquals(devis.getMontant(),5150.0);
		assertEquals(devis.getListeDetail().size(),4);
	}
	
	//@Disabled
	@Test @Order(3)
	public void test3_supprimerDevis(){
		try {
			devis.supprimerLigneDetail(devis.retrouverLigneDetail(3));
		} catch (ExceptionDocument e) {
			e.printStackTrace();
		}
		assertEquals(devis.toString(),"Devis-1, Nom du client: THOMAS Paul Montant: 3150.0");
